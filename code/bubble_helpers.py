...
class BubblesGroup:
    ...
    def __init__(self, bubbles) -> None:
        ...
        self.all_bubbles = bubbles
        self.bubbles = bubbles
        if len(bubbles) > 0:
            self.kd_tree = spatial.KDTree([b.pos for b in bubbles])
        else:
            self.kd_tree = None
    ...
    def get_bubble_containing_pt(self, point):
        sel_bubble = None
    ...
    def get_all_bubbles_of_state(self, state):
        return [b for b in self.all_bubbles if b.state == state]

    def set_bubble_state_containing_pt(self, point, state):
        b = self.get_bubble_containing_pt(point)
        if b is not None:
            b.state = state

    def set_bubble_state_in_roi(self, roi, state):
        if len(roi) > 0:
            for b in self.bubbles:
    ...
    def correlate_ids_to_bubbles(self, other_bubbles):
        # take the smaller bunch
        if len(other_bubbles) < len(self.bubbles):
            # assign other bubble's id to curr bubbles
            for other_b in other_bubbles:
                dist, self_nn_b = self.get_nearest_bubble(other_b.pos)
                self_nn_b.id = other_b.id
        else:
            # receive other bubble's id to curr bubbles
            for self_b in self.bubbles:
                dist, other_nn_b = other_bubbles.get_nearest_bubble(self_b.pos)
    ...
    def update_bubbles_to_new_markers(self, new_markers, frame_idx):
        # associate bubbles once temporal associations are complete
        # new markers: [(x1, y1, r1), (x2,,y2,,r2), ...]
        # [:, :2] --> [(x1,y1), (x2,y2), ...]

        # empty set of new markers: ignore
        ...
        for new_idx, m in enumerate(new_markers):
            ...
            if new_idx == nn_new_idx and dist1 < 200:
            ...
            # the prev and curr nearest neighbors do not agree
                ...
                new_b = Bubble(m[0], m[1], m[2], frame_idx)
                new_bubbles.append(new_b)
                self.all_bubbles.append(new_b)
    ...
    def get_nearest_bubble_to_pt(self, point):
        # print('pos:', pos)
        d, i = self.kd_tree.query(point, k=1)
        return d, self.bubbles[i]

    def get_self_neighbors(self, num_neighbors):
        # add 1 b/c it'll count the current bubble as neighbor
        dist_list, neighbor_idx_list = self.kd_tree.query(self.bubbles,
                                                          k=num_neighbors + 1)
...
def get_markers_from_label(labeled_frame, min_area=1, fit_circle='area'):
    new_markers_pts = []
