...
class FilterGroup(GroupParameter):
    ...
    def preprocess(self, frame):
        self.preview_frames['Original'] = frame
        for f in self.children():
            if isinstance(f, Filter) and f.child('Toggle').value():
                frame = f.process(frame)
                self.preview_frames[f.name()] = frame
        return frame

    # returns the selected preview if no parameters given
    # returns the preview of the given name if given
    def get_preview(self, name=None):
        ...
        if name is None:
            name = self.child('view_list').value()
        return self.preview_frames[name].copy()
